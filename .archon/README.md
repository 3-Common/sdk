...
- **`--branch`** names the worktree Archon creates for the run. The PR branch this workflow generates is always `feat/<domain>`, cut fresh from `origin/main` at publish time.
...
8. **publish-commit**: commits per language + the shared conformance scenarios and pushes `feat/<domain>`. **Only runs if all three validations pass**. If any fails, nothing is committed or pushed and no PR is opened.
