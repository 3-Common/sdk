...
paths or names from the raw invocation message — it may be capitalized or a whole
sentence; only `domain` is canonical, and it is the exact path the publish step
stages.

## Inputs
- The API contract to wrap: `$ARTIFACTS_DIR/resource-spec.json` — the `domain`
...
## Steps
0. **Install dependencies** (the worktree has no `.venv`): `cd sdk-python && uv venv --python 3.10 .venv && uv pip install -e ".[dev]"`. Run all Python tooling below via `uv run --no-sync <tool>` — it uses this venv exactly as installed (cross-platform, no `Scripts/`-vs-`bin/` issues; and `--no-sync` avoids a re-sync that could prune the dev extras, and never writes `uv.lock`).
