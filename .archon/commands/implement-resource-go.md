...
contain hyphens.
Mirror the sibling's conventions. Do NOT derive paths or names from the raw invocation message — it may be
...
9. **Get to green** (run from `sdk-go/`): `gofumpt -w .`, then iterate until clean — `make ci` (vet + lint + race tests + coverage; the `conformance` package runs here too). Ensure examples compile (`go build ./examples/...`). Do not stop while anything is red.
